...
use crate::main_support::{
    bot_username, codex_request_and_cwd, rendered_or_default, runtime_paths,
};
...
pub(crate) fn build_runtime_components(
    ...
    vibes_codex::CodexExecRunner,
    BotTopicManager,
    AppController<vibes_store::SqliteBindingStore, vibes_codex::CodexExecRunner, BotTopicManager>,
)> {
    let store = vibes_store::SqliteBindingStore::open(db_path)
        .with_context(|| format!("failed to open sqlite store at {db_path}"))?;
    let runtime = vibes_codex::CodexExecRunner::default();
    let topics = BotTopicManager { bot: bot.clone() };
    let controller_store = vibes_store::SqliteBindingStore::open(db_path)
        .with_context(|| format!("failed to open sqlite store at {db_path}"))?;
