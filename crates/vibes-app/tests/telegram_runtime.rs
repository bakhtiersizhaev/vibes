...
#[test]
fn sends_reply_for_new_session_update_from_trimmed_caption_in_direct_chat() {
    let controller = controller();
...
#[test]
fn returns_request_error_when_trimmed_caption_new_session_reply_send_fails_in_direct_chat() {
    let controller = controller();
    let requester = FakeRequester {
        sent: Mutex::new(Vec::new()),
        fail: Mutex::new(Some("send boom".to_owned())),
...
#[test]
fn sends_reply_for_new_session_update_from_caption_in_direct_chat() {
    let controller = controller();
    let requester = FakeRequester::default();
    let update = parse_update(
        r#"{
...
#[test]
fn returns_request_error_when_direct_caption_new_session_reply_send_fails() {
    let controller = controller();
    let requester = FakeRequester {
        sent: Mutex::new(Vec::new()),
        fail: Mutex::new(Some("send boom".to_owned())),
...
#[test]
fn returns_request_error_when_caption_new_session_reply_send_fails() {
    let controller = controller();
    let requester = FakeRequester {
        sent: Mutex::new(Vec::new()),
        fail: Mutex::new(Some("send boom".to_owned())),
...
#[test]
fn sends_reply_for_new_session_update_from_caption() {
    let controller = controller();
    let requester = FakeRequester::default();
    let update = parse_update(
        r#"{
...
#[test]
fn sends_reply_for_new_session_update_from_trimmed_caption_inside_existing_topic() {
    let controller = controller();
    let requester = FakeRequester::default();
    let update = parse_update(
        r#"{
...
#[test]
fn falls_back_to_chat_reply_for_trimmed_caption_new_session_reply_inside_existing_topic() {
    let controller = controller();
...
#[test]
fn returns_request_error_when_trimmed_caption_new_session_reply_thread_fallback_also_fails_inside_existing_topic()
 {
    let controller = controller();
...
#[test]
fn sends_reply_for_new_session_update_inside_existing_topic() {
    let controller = controller();
    let requester = FakeRequester::default();
    let update = parse_update(
        r#"{
...
#[test]
fn returns_request_error_when_caption_new_session_reply_send_fails_inside_existing_topic() {
    let controller = controller();
    let requester = FakeRequester {
        sent: Mutex::new(Vec::new()),
        fail: Mutex::new(Some("send boom".to_owned())),
...
#[test]
fn falls_back_to_chat_reply_for_caption_new_session_reply_inside_existing_topic() {
    let controller = controller();
    let requester = ThreadFailRequester::default();
    let update = parse_update(
        r#"{
...
#[test]
fn returns_request_error_when_caption_new_session_reply_thread_fallback_also_fails_inside_existing_topic()
 {
    let controller = controller();
    let requester = ThreadThenChatFailRequester::default();
    let update = parse_update(
...
#[test]
fn returns_request_error_when_new_session_reply_send_fails_inside_existing_topic() {
    let controller = controller();
    let requester = FakeRequester {
        sent: Mutex::new(Vec::new()),
        fail: Mutex::new(Some("send boom".to_owned())),
...
#[test]
fn falls_back_to_chat_reply_for_new_session_reply_inside_existing_topic() {
    let controller = controller();
    let requester = ThreadFailRequester::default();
    let update = parse_update(
        r#"{
...
#[test]
fn returns_request_error_when_new_session_reply_thread_fallback_also_fails_inside_existing_topic_text()
 {
    let controller = controller();
    let requester = ThreadThenChatFailRequester::default();
    let update = parse_update(
...
#[test]
fn topic_caption_prompt_execution_failure_propagates_request_send_failure() {
    let store = InMemoryBindingStore::default();
    store.upsert_binding(vibes_core::SessionBinding {
        scope: vibes_core::ChatScope::Topic {
            chat_id: -1001293752024,
...
#[test]
fn topic_caption_execution_failure_returns_request_error_when_thread_fallback_also_fails() {
...
#[test]
fn trimmed_caption_prompt_executes_and_replies_in_direct_chat() {
    let store = InMemoryBindingStore::default();
    store.upsert_binding(vibes_core::SessionBinding {
        scope: vibes_core::ChatScope::Direct(408258968),
        session: SessionHandle {
    ...
    assert_eq!(target.chat_id, 408258968);
    assert_eq!(target.message_thread_id, None);
    assert_eq!(text, "trimmed caption direct transcript");

    let sent = requester.sent.lock().expect("fake requester lock poisoned");
    assert_eq!(sent.len(), 1);
    assert_eq!(sent[0].0, 408258968);
    assert_eq!(sent[0].1, None);
    assert_eq!(sent[0].2, "trimmed caption direct transcript");
}

#[test]
fn trimmed_caption_prompt_execution_failure_propagates_request_send_failure_in_direct_chat() {
    let requester = FakeRequester {
        fail: Mutex::new(Some("reply send boom".to_owned())),
        sent: Mutex::new(Vec::new()),
    };
...
#[test]
fn trimmed_caption_prompt_result_propagates_request_send_failure_in_topics() {
    let requester = FakeRequester {
        sent: Mutex::new(Vec::new()),
        fail: Mutex::new(Some("reply send boom".to_owned())),
    };
    let executor = FakeExecutor {
        response: Mutex::new(Ok("trimmed caption topic transcript".to_owned())),
    };
    let outcome = RuntimeOutcome::PromptReady {
        target: ReplyTarget {
            chat_id: -1001293752024,
...
#[test]
fn returns_topic_prompt_ready_from_trimmed_caption_without_text() {
    let store = InMemoryBindingStore::default();
    store.upsert_binding(vibes_core::SessionBinding {
        scope: vibes_core::ChatScope::Topic {
            chat_id: -1001293752024,
...
#[test]
fn trimmed_caption_prompt_executes_and_replies_back_into_same_thread() {
    let store = InMemoryBindingStore::default();
    store.upsert_binding(vibes_core::SessionBinding {
        scope: vibes_core::ChatScope::Topic {
            chat_id: -1001293752024,
    ...
    assert_eq!(target.chat_id, -1001293752024);
    assert_eq!(target.message_thread_id, Some(900));
    assert_eq!(text, "trimmed caption topic transcript");

    let sent = requester.sent.lock().expect("fake requester lock poisoned");
    assert_eq!(sent.len(), 1);
    assert_eq!(sent[0].0, -1001293752024);
    assert_eq!(sent[0].1, Some(900));
    assert_eq!(sent[0].2, "trimmed caption topic transcript");
}

#[test]
fn trimmed_caption_prompt_falls_back_to_chat_reply_when_thread_send_fails() {
    let requester = ThreadFailRequester::default();
    let executor = FakeExecutor {
        response: Mutex::new(Ok(
            "trimmed caption topic transcript after fallback".to_owned()
        )),
    };
    let outcome = RuntimeOutcome::PromptReady {
        target: ReplyTarget {
    ...
    let completed =
        run_ready(complete_runtime_outcome(&requester, &executor, outcome)).expect("fallback ok");
    ...
    assert_eq!(target.chat_id, -1001293752024);
    assert_eq!(target.message_thread_id, Some(900));
    assert_eq!(text, "trimmed caption topic transcript after fallback");
    ...
    assert_eq!(sent[0].0, -1001293752024);
    assert_eq!(sent[0].1, None);
    assert_eq!(sent[0].2, "trimmed caption topic transcript after fallback");
}

#[test]
fn trimmed_caption_prompt_execution_failure_falls_back_to_chat_reply_when_thread_send_fails() {
    let requester = ThreadFailRequester::default();
    let executor = FakeExecutor {
        response: Mutex::new(Err("boom".to_owned())),
    };
...
#[test]
fn trimmed_caption_prompt_result_returns_request_error_when_thread_fallback_also_fails() {
    let requester = ThreadThenChatFailRequester::default();
    let executor = FakeExecutor {
        response: Mutex::new(Ok("trimmed caption topic transcript".to_owned())),
    };
    ...
    let err = run_ready(complete_runtime_outcome(&requester, &executor, outcome))
        .expect_err("double failure expected");
    assert!(
        err.to_string()
            .contains("telegram request failed: chat send boom")
    );
...
#[test]
fn trimmed_caption_prompt_execution_failure_returns_request_error_when_thread_fallback_also_fails()
{
    let requester = ThreadThenChatFailRequester::default();
    let executor = FakeExecutor {
        response: Mutex::new(Err("boom".to_owned())),
...
#[test]
fn trimmed_caption_prompt_execution_failure_propagates_request_send_failure_in_topics() {
    let requester = FakeRequester {
        fail: Mutex::new(Some("reply send boom".to_owned())),
        sent: Mutex::new(Vec::new()),
    };
...
#[test]
fn trimmed_caption_prompt_execution_failure_replies_back_into_same_thread() {
    let requester = FakeRequester::default();
    let executor = FakeExecutor {
        response: Mutex::new(Err("boom".to_owned())),
    };
...
#[test]
fn topic_caption_prompt_execution_failure_returns_request_error_when_thread_fallback_also_fails() {
    let requester = ThreadThenChatFailRequester::default();
    let executor = FakeExecutor {
        response: Mutex::new(Err("boom".to_owned())),
    };
    ...
    let err = run_ready(complete_runtime_outcome(&requester, &executor, outcome))
        .expect_err("double send failure expected");
    assert!(
        err.to_string()
            .contains("telegram request failed: chat send boom")
    );
...
#[test]
fn topic_caption_prompt_execution_failure_replies_back_into_same_thread() {
    let store = InMemoryBindingStore::default();
    store.upsert_binding(vibes_core::SessionBinding {
        scope: vibes_core::ChatScope::Topic {
            chat_id: -1001293752024,
...
#[test]
fn returns_request_error_when_trimmed_caption_resume_reply_send_fails_in_direct_chat() {
    let store = InMemoryBindingStore::default();
    store.upsert_binding(vibes_core::SessionBinding {
        scope: vibes_core::ChatScope::Direct(408258968),
        session: SessionHandle {
...
#[test]
fn returns_request_error_when_direct_caption_resume_reply_send_fails() {
    let store = InMemoryBindingStore::default();
    store.upsert_binding(vibes_core::SessionBinding {
        scope: vibes_core::ChatScope::Direct(408258968),
        session: SessionHandle {
...
#[test]
fn sends_resume_reply_from_trimmed_caption_into_existing_topic_thread() {
    let controller = controller();
    let requester = FakeRequester::default();
    let update = parse_update(
        r#"{
...
#[test]
fn sends_resume_reply_from_caption_into_existing_topic_thread() {
    let controller = controller();
    let requester = FakeRequester::default();
    let update = parse_update(
        r#"{
...
#[test]
fn falls_back_to_chat_reply_for_trimmed_caption_resume_reply_when_thread_send_fails() {
    let controller = controller();
    let requester = ThreadFailRequester::default();
    let update = parse_update(
        r#"{
...
#[test]
fn returns_request_error_when_trimmed_caption_resume_reply_send_fails() {
    let controller = controller();
    let requester = FakeRequester {
        sent: Mutex::new(Vec::new()),
        fail: Mutex::new(Some("send boom".to_owned())),
...
#[test]
fn falls_back_to_chat_reply_for_caption_resume_reply_when_thread_send_fails() {
    let controller = controller();
    let requester = ThreadFailRequester::default();
    let update = parse_update(
        r#"{
...
#[test]
fn returns_request_error_when_trimmed_caption_resume_reply_thread_fallback_also_fails() {
    let controller = controller();
    let requester = ThreadThenChatFailRequester::default();
    let update = parse_update(
        r#"{
...
#[test]
fn topic_prompt_result_returns_request_error_when_thread_fallback_also_fails() {
    let requester = ThreadThenChatFailRequester;
    let executor = FakeExecutor {
        response: Mutex::new(Ok("topic transcript".to_owned())),
    };
...
#[test]
fn topic_prompt_execution_failure_returns_request_error_when_thread_fallback_also_fails() {
    let requester = ThreadThenChatFailRequester;
    let executor = FakeExecutor {
        response: Mutex::new(Err("boom".to_owned())),
    };
